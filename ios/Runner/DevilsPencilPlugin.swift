...
/// `devils_book/pencil_interaction` EventChannel.
@objc class DevilsPencilPlugin: NSObject, FlutterPlugin, FlutterStreamHandler {
  static let channelName = "devils_book/pencil_interaction"

  private var eventSink: FlutterEventSink?
  private var pencilInteraction: UIPencilInteraction?
  private var rollRecognizer: DevilsBarrelRollGestureRecognizer?
  ...
  static func register(with registrar: FlutterPluginRegistrar) {
    ...
    let instance = DevilsPencilPlugin()
  ...
  private func attachToPencilInteraction() {
    ...
    if #available(iOS 16.0, *) {
      let recognizer = DevilsBarrelRollGestureRecognizer(onRollChanged: { [weak self] angle in
...
@available(iOS 12.1, *)
extension DevilsPencilPlugin: UIPencilInteractionDelegate {
...
class DevilsBarrelRollGestureRecognizer: UIGestureRecognizer {
